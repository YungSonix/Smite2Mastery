...
/** Build cumulative stat points: Base (+ starters) → each final item in slot order. */
export function buildItemProgressionSteps(finalItems = [], startingItems = []) {
  const starters = startingItems.filter(Boolean);
  const equipped = [...starters];
  const steps = [
    {
      id: starters.length ? 'starters' : 'base',
      label: starters.length ? 'Starters' : 'Base',
      shortLabel: starters.length ? 'Start' : 'Base',
      items: [...equipped],
    },
  ];
...
export function computeBuildProgressionSeries(god, godLevel, finalItems = [], startingItems = [], passiveOptions = {}) {
  const steps = buildItemProgressionSteps(finalItems, startingItems);
  ...
  return steps.map((step) => {
    const { totalStats } = computeTotalBuildStats(god, godLevel, step.items, passiveOptions);
