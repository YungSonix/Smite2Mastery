import {
  computeItemPassiveBonuses,
  computeDerivedItemPassiveBonuses,
  getStatsFromItemsPercentBonus,
  applyStatsFromItemsPercentMultiplier,
} from './customBuildItemPassives';
import { computeGodPassiveStatBonuses } from './customBuildGodPassives';
...
export function computeTotalBuildStats(god, godLevel, equippedItems = [], passiveOptions = {}) {
  ...
  const filteredItems = equippedItems.filter(Boolean);

  // % Stats from Items (Shell of Rebuke, Agility Greaves, Aladdin's Lamp, etc.) — stacks additively.
  const statsFromItemsPct = getStatsFromItemsPercentBonus(filteredItems, godLevel);
  applyStatsFromItemsPercentMultiplier(stats, baseStats, statsFromItemsPct);

  const passiveBonuses = computeItemPassiveBonuses(filteredItems, {
    ...passiveOptions,
    godLevel,
    statsFromItemsPercent: statsFromItemsPct,
  });

  const applyPassiveBonus = (bonuses) => {
    Object.keys(bonuses).forEach((k) => {
      const add = bonuses[k];
      if (typeof add === 'number' && Number.isFinite(add) && add !== 0) {
        if (k === 'Mana') {
          stats.MaxMana = (stats.MaxMana || 0) + add;
        } else if (k === 'BasicDamageFlat') {
          stats.BasicDamage = (stats.BasicDamage || 0) + add;
          stats['Attack Damage'] = (stats['Attack Damage'] || 0) + add;
        } else if (k === 'AttackDamageMultiplier' || k === 'BasicDamageFlat') {
          // handled below
        } else if (k === 'AttackSpeedPercent' || k === 'MovementSpeed') {
  ...
  applyPassiveBonus(passiveBonuses);

  const godBonuses = computeGodPassiveStatBonuses(god, godLevel, passiveOptions);
  applyPassiveBonus(godBonuses);

  const derivedBonuses = computeDerivedItemPassiveBonuses(stats, filteredItems);
  applyPassiveBonus(derivedBonuses);

  const attackDamageMult =
    (passiveBonuses.AttackDamageMultiplier || 1) * (derivedBonuses.AttackDamageMultiplier || 1);
  ...
  if (attackDamageMult !== 1) {
    stats.BasicDamage = Math.floor(stats.BasicDamage * attackDamageMult);
  }
