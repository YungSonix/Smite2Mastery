...
/** Higher pct wins; same pct → earlier submit wins (returns negative if a is better). */
function cmpBestAttempt(a, b) {
  const pctA = pctForResponse(a);
  const pctB = pctForResponse(b);
  const aNull = pctA == null;
  const bNull = pctB == null;
  if (aNull && bNull) return 0;
  if (aNull) return 1;
  if (bNull) return -1;
  if (pctB !== pctA) return pctB - pctA;
  const msA = submittedMs(a);
  const msB = submittedMs(b);
