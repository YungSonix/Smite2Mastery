...
import { ensureAppWriteSession } from './appAuth';
...
async function requireWriteSession(username) {
  const u = String(username || '').trim();
  if (!u) throw new Error('Not logged in');
  const session = await ensureAppWriteSession(u);
  if (!session.ready) {
    throw new Error('Sign in again to sync guides with the server.');
  }
  return u;
}
...
export async function insertCommunityGuide(username, fields) {
  const client = getClient();
  if (!client) throw new Error('Supabase not configured');
  const u = await requireWriteSession(username);
