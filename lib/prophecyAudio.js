...
function stripFoilFromName(name) {
  return String(name || '')
    .replace(/\s*\[foil\]\s*$/i, '')
    .replace(/^foil\s+/i, '')
    .trim();
...
/** Resolve god folder name for VOX — alt/foil cards must pass `baseGodName` on the object. */
export function resolveProphecyVoxGodName(input) {
  if (input == null) return '';
  if (typeof input === 'object') {
    const base = String(input.baseGodName || '').trim();
    if (base) return base;
    return stripFoilFromName(input.name);
  }
  return stripFoilFromName(input);
}

export async function playVOX(godNameOrCard, category, opts = {}) {
  try {
    const godName = resolveProphecyVoxGodName(godNameOrCard);
    if (!godName) return false;

    const categoryKey = String(category || '').toLowerCase();
    const bypassCooldownCategory =
      categoryKey === 'select' || categoryKey === 'intro' || categoryKey === 'gruntattack' || categoryKey === 'grunthit';
    ...
    /** Some gods ship empty default lines — wait for real duration before playing. */
    const needsLoadProbe = categoryKey === 'select' || categoryKey === 'intro';
    if (needsLoadProbe) {
