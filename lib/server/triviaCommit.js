const { scoreAnswersWithVariants, playerFacingScore, applyVariant, normalizeStoredIp, resolveVariantMap } = require('./triviaApi');
...
const { queueQuestionResponses } = require('./triviaQuestionResponses');
const { queuePlayerProfileUpsert } = require('./triviaPlayerProfiles');
...
async function commitGuestAttempt(sb, {
  ...
  try {
    ...
    if (live?.variant_map && typeof live.variant_map === 'object') {
      sessionVariantMap = live.variant_map;
    }
  ...
  const map = resolveVariantMap({
    questions: questions || [],
    slug: quiz.slug,
    discord,
    clientMap: variantMap,
    sessionMap: sessionVariantMap,
  });
  ...
  if (!isTestTake) {
    queuePlayerProfileUpsert(sb, discord);
  }
