...
const { fetchAppProfileTitlesByDiscordKeys } = require('./triviaAppProfileTitles');
...
async function fetchHostAnalytics(sb, username, { syncProfiles = false } = {}) {
  const { data: quizzes, error } = await sb
    .from('trivia_quizzes')
    .select('id, slug, title, updated_at, settings')
  ...
  let appProfileTitles = {};
  if (hostKeys.length) {
    try {
      appProfileTitles = await fetchAppProfileTitlesByDiscordKeys(sb, hostKeys);
    } catch (titleErr) {
      console.warn('trivia app profile titles', titleErr.message);
    }
  }
...
async function handleHostClassroomPost(sb, username, body) {
  ...
  if (action === 'classroom-points-bulk') {
    const delta = Number(body.delta);
    const keys = Array.isArray(body.discordKeys)
      ? body.discordKeys
      : Array.isArray(body.discord_keys)
        ? body.discord_keys
        : [];
    const result = await adjustClassroomBonusBulk(sb, keys, delta);
