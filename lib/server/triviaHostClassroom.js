...
async function fetchHostAnalytics(sb, username, { syncProfiles = true } = {}) {
  ...
  try {
    ...
    if (!pErr) {
      ...
      playerProfiles = (profiles || []).filter((p) => hostKeys.has(p.discord_key));
    } else if (String(pErr.message || '').includes('trivia_player_profiles')) {
