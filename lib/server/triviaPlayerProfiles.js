...
const { classroomAutoPointsTotal } = require('../classroomScoring');
...
function profileRowToDb(
  ...
  const pts =
    classroomPointsOverride != null
      ? Math.round(Number(classroomPointsOverride) || 0)
      : Number(row.classroom_points) || 0;
  const out = {
...
async function fetchQuizzesForScoring(sb, quizIds) {
  if (!quizIds?.length) return [];
  const { data, error } = await sb
    .from('trivia_quizzes')
...
async function fetchProductionResponsesForQuizIds(sb, quizIds) {
  if (!quizIds?.length) return [];
  const { data, error } = await sb
    ...
    .in('quiz_id', quizIds);
  if (error) throw error;
  return (data || []).filter((r) => !responseIsTestRow(r));
}

function classroomPointsForPlayer(discordKey, prodResponses, quizzes) {
  return classroomAutoPointsTotal(discordKey, prodResponses, quizzes);
}
...
async function upsertPlayerProfileForDiscord(sb, discordUsername) {
  ...
  const [questions, quizzes, allProdForQuizzes] = await Promise.all([
    fetchQuestionsForQuizIds(sb, quizIds),
    fetchQuizzesForScoring(sb, quizIds),
    fetchProductionResponsesForQuizIds(sb, quizIds),
  ]);
  ...
  if (!row) return null;
  const autoPoints = classroomPointsForPlayer(discordKey, allProdForQuizzes, quizzes);
  ...
  const { data, error } = await sb
    ...
    .upsert(
      profileRowToDb(row, {
        ...
        classroomPointsOverride: autoPoints,
      }),
...
async function syncAllPlayerProfiles(sb) {
  ...
  const [questions, quizzes] = await Promise.all([
    fetchQuestionsForQuizIds(sb, allQuizIds),
    fetchQuizzesForScoring(sb, allQuizIds),
  ]);
  const questionById = questionByIdFromList(questions);
  ...
  for (const [discordKey, { discord, rows }] of byKey) {
    ...
    if (row) {
      ...
      const autoPoints = classroomPointsForPlayer(discordKey, prod, quizzes);
      profiles.push(
        profileRowToDb(row, {
