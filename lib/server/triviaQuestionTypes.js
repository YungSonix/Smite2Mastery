...
const { lifelineMultiplier, extractLifelines } = require('./triviaHints');
...
function normalizeType(type, q) {
  ...
  if (t === 'drag_drop') {
    if (q && q.options && !Array.isArray(q.options)) return 'categorize';
    return 'ordering';
  }
  return t;
}
...
function sameSet(a, b) {
  const A = new Set(asList(a).map(normalize));
  const B = new Set(asList(b).map(normalize));
  if (!A.size || A.size !== B.size) return false;
  for (const x of A) if (!B.has(x)) return false;
  return true;
}

function isBlankAnswer(raw) {
...
function gradeOne(q, raw) {
  ...
  const type = normalizeType(q.type, q);
  ...
  if (type === 'multiple_selection') {
    ...
    if (q.meta?.allow_partial_credit) {
      let tp = 0;
      let fp = 0;
      for (const i of got) {
        if (want.has(i)) tp += 1;
      ...
      return Math.max(0, (tp - fp) / want.size);
    }
  ...
  if (type === 'matching' || type === 'categorize') {
    ...
    const keys = Object.keys(map).filter((k) => asList(map[k]).length);
    if (!keys.length) return null;
    const given = raw && typeof raw === 'object' ? raw : {};
    return keys.every((k) => sameSet(given[k], map[k]));
  }

  if (type === 'ordering') {
    const want = Array.isArray(correct.order)
      ? correct.order
    ...
    return want.every((w, i) => normalize(w) === normalize(got[i]));
  }
...
function scoreAnswers(questions, answers) {
  ...
  for (const q of questions || []) {
    ...
    if (isManualType(q.type)) {
      maxScore += pts;
      perQuestion[q.id] = isBlankAnswer(raw) ? 0 : null;
      continue;
    }
    ...
    maxScore += pts;
    const frac = typeof ok === 'number' ? Math.max(0, Math.min(1, ok)) : ok ? 1 : 0;
    perQuestion[q.id] = frac;
    score += frac * pts * lifelineMultiplier(usedMap[q.id]);
  }
  return { score, maxScore, perQuestion, lifelines: usedMap };
}
...
function playerFacingScore(questions, graded) {
  ...
  for (const q of questions || []) {
    ...
    if (Number.isFinite(frac)) {
      score += Math.max(0, Math.min(1, frac)) * pts * lifelineMultiplier(graded?.lifelines?.[q.id]);
    }
  }
  return {
    score,
