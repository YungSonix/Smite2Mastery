...
/**
 * Force full or zero credit on specific question(s) for one submission.
 * Does not re-run auto grading — host override.
 */
function forceQuestionCreditOneResponse({ questions, response, questionIds, mode }) {
  const answers = response?.answers && typeof response.answers === 'object' ? response.answers : {};
  const prev = response?.per_question && typeof response.per_question === 'object'
    ? { ...response.per_question }
    : {};
  const next = { ...prev };
  const limit = new Set((questionIds || []).map(String));
  const m = String(mode || '').toLowerCase();
  if (m !== 'full' && m !== 'zero') {
    const err = new Error('mode must be full or zero');
    err.status = 400;
    throw err;
  }
  ...
  for (const q of questions || []) {
    ...
    const pts = Number.isFinite(Number(q.points)) ? Number(q.points) : 1;
    if (pts <= 0) continue;
    next[qid] = m === 'full' ? pts : 0;
  }

  const { score, maxScore } = scoreFromPerQuestion(questions, next, answers);
  const changed =
    perQuestionChanged(prev, next, [...limit]) ||
    Number(response?.score) !== Number(score) ||
    Number(response?.max_score) !== Number(maxScore);
  ...
  return {
    ...
    score,
    maxScore,
    variantMap: extractVariantMap(answers) || {},
    answers,
    changed,
  };
}
...
async function adjustQuestionCreditForQuiz(sb, { quiz, questions, questionIds, mode }) {
  const m = String(mode || '').toLowerCase();
  if (m === 'regrade') {
    return regradeQuizResponses(sb, { quiz, questions, questionIds, responses: await loadQuizResponses(sb, quiz.id) });
  }
  if (m !== 'full' && m !== 'zero') {
    const err = new Error('mode must be full, zero, or regrade');
    err.status = 400;
    throw err;
  }
  const ids = (questionIds || []).map(String).filter(Boolean);
  if (!ids.length) {
    const err = new Error('questionId required');
    err.status = 400;
    throw err;
  }
  const responses = await loadQuizResponses(sb, quiz.id);
  if (!responses.length) return { total: 0, updated: 0, unchanged: 0, errors: 0, mode: m };
  ...
  for (const row of responses) {
    try {
      ...
      errors += 1;
      console.warn('trivia question credit', row?.id, err?.message || err);
    }
  }

  return { total: responses.length, updated, unchanged, errors, mode: m };
}
...
module.exports = {
  answerKeyChanged,
  regradeOneResponse,
  forceQuestionCreditOneResponse,
  regradeQuizResponses,
  regradeOwnedQuiz,
  adjustQuestionCreditForQuiz,
