...
function extractResponseMeta(answers, submittedAt) {
  ...
  const hasPresence = ans.__presence && typeof ans.__presence === 'object';
  const pres = hasPresence ? ans.__presence : {};
  return {
    duration_ms: normalizeDurationMs(ans.__duration_ms, submittedAt, ans.__started_at),
    tab_away_count: hasPresence ? Number(pres.hidden_count) || 0 : null,
    left_page: hasPresence ? Boolean(pres.left_page) : null,
  };
}
