...
| Folder aliases | `scripts/lib/godRenderFolderAliases.js` (e.g. `cernennos` → Cernunnos, `morgan le fay` → Morgan Le Fay) |
...
- `screenshotTag` shape: `scripts/lib/godRenderScreenshotTags.js` — `fileName`, `target` (`skin` | `variant`), `appliedTo`, optional `cost`, `tier`
...
```bash
node scripts/apply-vision-tags-agent-f.js --write
node scripts/rebuild-god-render-audit.js
```

Log: `scripts/.vision-tag-agent-f.log`
...
Tag **one pantheon at a time** (~5 gods/day vision-only). Greek log: `scripts/.vision-tag-greek.log`.
