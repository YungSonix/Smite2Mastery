/**
 * Group flat skin `variants[]` into saga forms + prism rows for the skin data viewer.
 * Saga skins (e.g. Dark Tyrant) have multiple unlockable forms; each form has its own prism set.
 */
(function (global) {
  ...
  /** "Emberlord" matches skinName "Emberlord Ashen Ascendant", etc. */
  ...
  /**
   * @returns {{ hasSagaForms: boolean, hasProgressionForms: boolean, progressionForms: object[], defaultPrisms: object[], forms: { formName, formEntry, prisms }[] }}
   */
  function groupSkinVariants(skin) {
  ...
  function prismDisplayLabel(variant) {
    const name = variant._prismDisplayName || variant.name || variant.loadoutMeta?.displayName || 'Prism';
    const code = extractPrismCode(variant);
    if (code && !normalizeKey(code).includes(normalizeKey(name))) {
      return { title: name, alias: code };
    }
    return { title: name, alias: code };
  }
  ...
  function skinFromSagaForm(parentSkin, form) {
    ...
    return {
      ...parentSkin,
      ...assets,
      skinKey: `${parentSkin.skinKey}:${slugFormKey(form.formName)}`,
      skinName: form.formName,
      name: form.formName,
      _sagaFormSkin: true,
      _sagaParentSkinKey: parentSkin.skinKey,
      _sagaParentSkinName: parentSkin.skinName,
      variants: prisms,
      isPrism: true,
  ...
  function expandSagaFormSkins(skin) {
  ...
  function compareSagaSkinsForDisplay(a, b) {
  ...
  function expandGodSkinsList(skins) {
    if (!Array.isArray(skins)) return [];
    return skins.flatMap((skin) => expandSagaFormSkins(skin));
  }

  global.SkinVariantGroups = {
    visibleVariants,
    extractPrismCode,
    groupSkinVariants,
    prismDisplayLabel,
