 ...
 * Mappings live in scripts/vision-tag-batch-c-mappings.json
 *
 *   node scripts/vision-tag-batch-c.js
 *   node scripts/vision-tag-batch-c.js --dry-run
 */
const fs = require('fs');
...
const LOG_PATH = path.join(PROJECT_ROOT, 'scripts', '.vision-tag-batch-c.log');
const MAP_PATH = path.join(PROJECT_ROOT, 'scripts', 'vision-tag-batch-c-mappings.json');
const BOOTSTRAP_PATH = path.join(PROJECT_ROOT, 'scripts', 'vision-tag-bootstrap-gods.json');
...
function main() {
  ...
  const mappingsPath = fs.existsSync(MAP_PATH)
    ? MAP_PATH
    : path.join(PROJECT_ROOT, 'scripts', 'vision-tag-batch-c-mappings.json');
